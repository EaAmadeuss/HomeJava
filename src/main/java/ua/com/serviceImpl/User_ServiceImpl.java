...
import ua.com.CRUD.dao.User_Dao;
import ua.com.enity.Role;
...
public class User_ServiceImpl implements User_Service, UserDetailsService{
	...
	public void sendMail(String content, String email, String mailBody) {
		...
		return new PasswordAuthentication("apleshop@gmail.com", "F.U.A.L.");
		}
		});
		try {
		MimeMessage message = new MimeMessage(session);
		message.setFrom(new InternetAddress("apleshop@gmail.com"));
		message.addRecipient(Message.RecipientType.TO, new InternetAddress(
		email));
		message.setSubject(content, "UTF-8");
		message.setText(mailBody);
		Transport.send(message);
		} catch (MessagingException ו) {
		ו.printStackTrace();
		}
